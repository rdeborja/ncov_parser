...
def get_qc_from_consensus_data(fasta, reference, consensus_suffix='.consensus.fa'):
    '''
    Since the <sample>.qc.csv file is not generated for ONT and Illumina runs,
    we obtain the required pct_n_bases and pct_covered_bases from the consensus.
    '''
    genome_length = 0
    qc_data = {}
    pct_n_bases = ''
...
def get_variant_counts(file, delimiter='\t'):
    '''
    ...
    num_iupac_snvs = 0
    with open(file) as file_p:
        csv_reader = csv.DictReader(file_p, delimiter=delimiter)
        for item in csv_reader:
            samplename = re.sub('^Consensus_', '', item['name'])
            if samplename in data:
                if is_variant_iupac(item['alt_allele']):
                    data[samplename]['num_consensus_iupac'] += 1
                elif is_variant_base(item['alt_allele']):
                    data[samplename]['num_consensus_snvs'] += 1
                else:
                    print("Invalid variant detected")
            else:
                if is_variant_iupac(item['alt_allele']):
                    data[samplename] = {'num_consensus_iupac': 1,
...
def is_variant_base(variant):
    '''
    A function to determine whether a variant is a valid nucleotide.
